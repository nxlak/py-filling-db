    ...
    for cart_id in cart_ids:
        if random.random() < 0.7:  # 70% вероятность, что корзина не пустая
            ...
            for product_id in products:
                ...
                total_price += product_id_to_price[product_id]
...
def fill_orders(user_ids, product_id_to_price):
    ...
    for user_id in user_ids:
        ...
        for _ in range(num_orders):
            ...
            time = fake_ru.time()
            data_orders.append((user_id, pay_method_id, date, time))

    if data_orders:
        values_list = []
        for order in data_orders:
            values_list.append(f"({order[0]}, {order[1]}, '{order[2]}', '{order[3]}')")

        query_orders = 'INSERT INTO orders(user_id, pay_method_id, date, time) VALUES ' + ', '.join(values_list) + ' RETURNING order_id;'
        ...
        idx_order = 0
        while idx_order < len(fetched_order_ids):
            ...
            for product_id in products:
                ...
                total_price += product_id_to_price[product_id]
            order_prices[order_id] = round(total_price, 2)
            idx_order += 1
